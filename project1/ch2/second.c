...
// your kernel module will report the number of seconds that have elapsed since the kernel module was first loaded. Be sure to remove /proc/seconds when the module is removed.
 ...
 static ssize_t proc_read(struct file *file, char __user *usr_buf, size_t count, loff_t *pos)
 {
         ...
         completed = 1;
         long current_seconds = jiffies/HZ;     
         long elapsed_minutes = (current_seconds - initial_seconds) / 60;
         rv = sprintf(buffer, "Current seconds since module loaded: %lu\nCurrent minutes: %lu\n", current_seconds - initial_seconds, elapsed_minutes);
